...
(define (main s)
  (big-bang s                           ; Saiki  
            ...
            (on-key handle_key)         ; Saiki KeyEvent -> Saiki  
            (on-mouse handle_click)))   ; Saiki Integer Integer MouseEvent -> Saiki    
...
;; moves saiki to the mouse's current x position
(check-expect (handle_click 10 50 0 "button-down") 50)
(check-expect (handle_click 50 10 0 "button-down") 10)
(check-expect (handle_click 0 50 0 "button-down") 50)

;(define (handle_click s x y me) 0)       ; stub

(define (handle_click s x y me)
  (cond [(mouse-event? me) x]
        [else s]))
